...
So `llama.cpp` becomes a second WebAssembly module. Emscripten builds it with a small C shim, and the Go code calls that module through JavaScript. The shim is in the `wasm` directory of the [llama-cpp-builder](https://github.com/hybridgroup/llama-cpp-builder) repository.
...
The GPU helps most on a page that takes images. Running an image through a model's projector takes a second or two on the GPU, compared to half a minute or more on the CPU.

## Why a Web Worker

The generation loop takes a long time. It would stop the page if it ran on the main thread. The worker lets the page keep drawing and accepting clicks.
...
The browser package doesn't support audio, video, LoRA adapters, saving state to a file, or quantization. It saves context state in memory.

The browser must download the model file before the first token. A 2 GB model is a 2 GB download.
