...
**Large, Small, and Tiny Language Models** read text and write text. A model with `instruct` or `it` in the name follows instructions. A model without it only continues the text.

**Vision Language Models (VLM)** read an image and text together. A VLM needs two files, the model file and the projector file. The name of a projector file starts with `mmproj`. The projector turns the image into tokens that the model reads.
...
The context is the model's memory for one conversation. It holds the prompt and the answer. `ContextParams.NCtx` sets the size in tokens.
...
An instruction model expects the text in a specific shape, with a marker before each user message and each assistant message. That shape is the chat template.
