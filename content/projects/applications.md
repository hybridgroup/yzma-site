---
title: "Applications"
linkTitle: "Applications"
type: "docs"
weight: 20
description: >
  Programs that people build with yzma.
---

### wasmVision

<a href="https://wasmvision.com" target="_blank">
<img src="https://github.com/wasmvision/wasmvision/blob/main/images/wasmvision-logo.png?raw=true" width="300" alt="wasmVision logo">
</a>

wasmVision is a computer vision processing engine with advanced algorithms and vision models for machine learning.

https://github.com/wasmvision/wasmvision

### NornicDB

<a href="https://github.com/orneryd/NornicDB" target="_blank">
<img src="https://raw.githubusercontent.com/orneryd/NornicDB/refs/heads/main/docs/assets/logos/nornicdb-logo.svg" width="300" alt="NornicDB logo">
</a>

NornicDB is a graph database for AI agents and knowledge systems.

https://github.com/orneryd/NornicDB

### Crush

<a href="https://github.com/charmbracelet/crush" target="_blank">
<img src="https://github.com/user-attachments/assets/cf8ca3ce-8b02-43f0-9d0f-5a331488da4b" width="300" alt="Crush logo">
</a>

Crush is agentic coding with style. It is built on Fantasy.

https://github.com/charmbracelet/crush

### floop

<a href="https://github.com/nvandessel/floop" target="_blank">
<img src="https://github.com/user-attachments/assets/bc695966-8c2b-4956-9b8b-e711333588e4" width="300" alt="floop logo">
</a>

floop gives AI coding agents a memory that lasts.

https://github.com/nvandessel/floop

### ImmyGo

<a href="https://immygo.app/" target="_blank">
<img src="https://raw.githubusercontent.com/amken3d/Immygo/refs/heads/master/assets/immygo-logo-sq.svg" width="300" alt="ImmyGo logo">
</a>

ImmyGo is a Go user interface framework that puts AI first. Describe what you want, and ImmyGo builds it. It is built on Gio, and it uses yzma, Ollama, and the Anthropic API for its local AI.

https://immygo.app/

### Pingo

<a href="https://github.com/amken3d/Pingo" target="_blank">
<img src="https://raw.githubusercontent.com/amken3d/Pingo/refs/heads/master/assets/Screenshot.png" width="300" alt="Pingo">
</a>

Pingo is a pin selector for the Raspberry Pi Pico and Pico 2. It is built with ImmyGo. The chat gives advice about pin selection, and yzma is one of its backends.

https://github.com/amken3d/Pingo

### unch

<a href="https://github.com/uchebnick/unch" target="_blank">
<img src="https://raw.githubusercontent.com/uchebnick/unch/refs/heads/main/docs/assets/unch-logo.svg" width="300" alt="unch logo">
</a>

unch provides semantic search for code symbols and documents.

https://github.com/uchebnick/unch

### Terminal agent

Terminal agent is an LLM agent that helps you from the terminal and in the terminal.

https://github.com/laszukdawid/terminal-agent

### Talking Heads From The Year 2053

<a href="https://github.com/talkingheads2053/talkingheads" target="_blank">
<img src="https://raw.githubusercontent.com/talkingheads2053/talkingheads/refs/heads/main/images/th2053-logo-red.png" width="300" alt="Talking Heads From The Year 2053 logo">
</a>

Physical AI that runs locally on the Arduino UNO Q, as you have never seen it before.

https://github.com/talkingheads2053/talkingheads

### OpenOcta

<a href="https://github.com/openocta/openocta" target="_blank">
<img src="https://raw.githubusercontent.com/openocta/openocta/refs/heads/main/imgs/readmePIC/QQ20260710-140337.png" width="300" alt="OpenOcta logo">
</a>

OpenOcta is a desktop IT operations agent for Windows and macOS.

https://github.com/openocta/openocta
