...
This command downloads all three builds, one for WebGPU, one with more than one thread, and one with a single thread. The JavaScript glue selects the best build that the page can run.
...
[Try it in your browser](/try/) runs a page like this now, with no install. See [Run yzma in a browser](/docs/tutorials/browser/) for a complete lesson, and [WebAssembly](/docs/concepts/webassembly/) for how it works.
